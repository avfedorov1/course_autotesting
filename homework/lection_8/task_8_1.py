...
def treatment_sum(our_tuple):
    """
    Функция проверяет переданные переменные и складывает их, если нельзя сложить, то выдает исключения.
    :param our_tuple: На вход поступает кортеж.
    :return: сумма либо исключения
    """
    try:
    ...
    # Если в кортеже 2 элемента и их нельзя сложить -> исключение возвращает строку 'Нельзя сложить эти данные'.
    except TypeError:
        return 'Нельзя сложить эти данные'
    # Если в кортеже меньше двух элементов -> исключение возвращает строку 'Недостаточно данных'.
    except IndexError:
