...
def repeats(our_str):
    """
    Функция считает, сколько раз каждый символ уже встречался
    ...
    dict_our_str = dict.fromkeys([letter for letter in our_str], 0)  # Создаем словарь с парой ключ (буквы слова): 0
    # Для каждой буквы принимаемой строки увеличиваем значение встречаемого ключа.
    # И в новую строку добавляем букву + _ + значение ключа из словаря
    for letter in our_str:
        dict_our_str[letter] = dict_our_str.get(letter) + 1
        new_str += letter + '_' + str(dict_our_str.get(letter))
    return new_str
