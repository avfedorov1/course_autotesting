...
def josephus_task(num_people, kill_num):
    ...
    survivor_list = [x + 1 for x in range(num_people)]  # генерируем список воинов по переданному их количеству
    while len(survivor_list) > 1:  # пока этот список больше 1 выполняем действия ниже
        for q in range(0, kill_num - 1):
            survivor_list.append(survivor_list[q])  # q = значение от 0 до kill_num - 1 => его кладем в конец списка
        del survivor_list[:kill_num]  # удаляем все до kill_num
    survivor = survivor_list[0]  # выводим значение единственного индекса (не стал преобразовывать)
    return survivor
