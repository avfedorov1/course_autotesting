...
def everything_for_your_cat(cats_data):
    """
    Функция оптимизирует хранение данных -> на одной строке для каждого владельца выводятся данные о всех его животных.
    :param cats_data: Список кортежей.
    :return: Каждый владелец с данными о его животных на новой строке
    """
    ...
    for tpl in cats_data:
        ...
        (cat, age, first_name, last_name) = tpl
        # Присваиваем значение для ключа
        key = first_name + ' ' + last_name
        # Присваиваем значение для значения ключа
        value = cat + ', ' + str(age)
        # Создаем новый словарь с парой ключ: значение
        new_dict = dict.fromkeys([key], value)
        # Если ключ присутствует в итоговом словаре,
        # то присваиваем ему новое значение и обновляем ключ в словаре.
        # Иначе просто обновляем словарь, добавляя новую пару
        if key in finish_dict.keys():
            new_value = finish_dict.get(key) + '; ' + new_dict.get(key)
            finish_dict[key] = new_value
        else:
            finish_dict.update(new_dict)
    # Итерируемся по ключ: значение и формируем итоговую строку
